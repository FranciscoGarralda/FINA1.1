...
interface IngresoCapitalDraftData {
  accountId: string;
  currencyId: string;
  format: string;
  amount: string;
...
export default function IngresoCapitalForm({ movementId, onDone, onCancel }: { movementId: string; onDone: () => void; onCancel: () => void }) {
  ...
  useEffect(() => {
    ...
    loadOperationDraft<IngresoCapitalDraftData>(movementId, 'INGRESO_CAPITAL')
      .then((draft) => {
  ...
  async function handleSubmit() {
    ...
    setSubmitting(true);
    try {
      await api.post(`/movements/${movementId}/ingreso-capital`, {
      ...
      setError(err?.message || 'Error al guardar el ingreso de capital.');
    } finally {
      setSubmitting(false);
    }
  }

  function buildDraftData(): IngresoCapitalDraftData {
  ...
  async function handleSaveDraft() {
    ...
    setSavingDraft(true);
    try {
      await saveOperationDraft(movementId, 'INGRESO_CAPITAL', buildDraftData());
  ...
  if (success) {
    return (
      <div className="border-t pt-4">
        <p className="text-green-700 font-medium mb-4">Ingreso de capital registrado correctamente.</p>
  ...
  return (
    <div className="border-t pt-4 space-y-6">
      ...
      <fieldset>
        <legend className="text-sm font-semibold text-gray-700 mb-2">Ingreso de capital</legend>
