...
// IN → ccSideIn, OUT → ccSideOut). Las no pendientes ya están saldadas en
// caja: NO debe registrarse cc_entries (regla 6 — sin doble impacto).
...
func TestSimulacionCompraTablaMaestra_DocumentacionFlujo(t *testing.T) {
	...
	b.WriteString("     CC && pending     → applyCCImpactTx(ccSideIn, \"Compra — divisa pendiente de cobro al cliente\")\n")
	...
	b.WriteString("     CC && pending     → applyCCImpactTx(ccSideOut, \"Compra — pago pendiente al cliente\")\n")
