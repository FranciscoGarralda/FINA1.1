...
func retiroCapitalHandler(svc *services.RetiroCapitalService) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		var input services.RetiroCapitalInput
		...
		if err != nil {
			handleRetiroCapitalError(w, err)
			return
		}
...
func handleRetiroCapitalError(w http.ResponseWriter, err error) {
	switch {
	...
	case errors.Is(err, services.ErrMovementTypeMismatch):
		RespondError(w, http.StatusBadRequest, "TYPE_MISMATCH", "El movimiento no es de tipo RETIRO_CAPITAL.")
	default:
