...
func compraHandler(svc *services.CompraService) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		var input services.CompraInput
...
func handleCompraError(w http.ResponseWriter, err error) {
	switch {
	case errors.Is(err, services.ErrCuadreNotMatch):
		RespondError(w, http.StatusBadRequest, "CUADRE_NOT_MATCH", "El total de salidas no coincide con el equivalente.")
	...
	case errors.Is(err, services.ErrNoOutLines):
		RespondError(w, http.StatusBadRequest, "NO_OUT_LINES", "Se requiere al menos una línea de salida.")
	...
	case errors.Is(err, services.ErrMovementTypeMismatch):
		RespondError(w, http.StatusBadRequest, "TYPE_MISMATCH", "El movimiento no es de tipo COMPRA.")
	default:
