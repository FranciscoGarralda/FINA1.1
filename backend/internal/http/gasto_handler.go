...
func gastoHandler(svc *services.GastoService) http.HandlerFunc {
...
func handleGastoError(w http.ResponseWriter, err error) {
	switch {
	...
	case errors.Is(err, services.ErrMovementTypeMismatch):
		RespondError(w, http.StatusBadRequest, "TYPE_MISMATCH", "El movimiento no es de tipo GASTO.")
	default:
