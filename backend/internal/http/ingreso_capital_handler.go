...
func ingresoCapitalHandler(svc *services.IngresoCapitalService) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		var input services.IngresoCapitalInput
		...
		if err != nil {
			handleIngresoCapitalError(w, err)
			return
		}
...
func handleIngresoCapitalError(w http.ResponseWriter, err error) {
	switch {
	...
	case errors.Is(err, services.ErrMovementTypeMismatch):
		RespondError(w, http.StatusBadRequest, "TYPE_MISMATCH", "El movimiento no es de tipo INGRESO_CAPITAL.")
	default:
